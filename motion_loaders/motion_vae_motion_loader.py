...
class MotionVAEGeneratedDataset(Dataset):
    ...
    def __init__(self, opt, num_motions, batch_size, device, label=None):
        ...
        prior_net = vae_models.GaussianGRU(opt.input_size, opt.dim_z, opt.hidden_size, 8,
        ...
        if opt.use_lie:
            ...
            decoder = vae_models.DecoderGRU(opt.input_size + opt.dim_z, opt.output_size, opt.hidden_size, 8,
                                            opt.decoder_hidden_layers,
                                            opt.num_samples, device)
